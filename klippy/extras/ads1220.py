...
class ADS1220:
    def __init__(self, config):
        ...
        self.idac_current_options = {
            'disabled': 0b0000, '10': 0b0001, '50': 0b0010,
            '100': 0b0011, '250': 0b0100, '500': 0b0101,
            '1000': 0b0110, '1500': 0b0111
        }
        self.idac_current = config.getchoice('idac_current', self.idac_current_options)
        ...
        self.idac1_routing = config.getchoice('idac1_routing', self.idac1_2_routing_options)
        self.idac2_routing = config.getchoice('idac2_routing', self.idac1_2_routing_options)

        # DRDY mode
        self.data_ready_mode = config.getboolean('data_ready_mode', default=False)
    ...
    def setup_chip(self):
        ...
        reg_values = [
            ...
            (self.vref << 6) | (self.fir << 4) | (int(self.psw) << 3) | self.idac_current, # Reg 2
            (self.idac1_routing << 5) | (self.idac2_routing << 2) | (self.data_ready_mode << 1) # Reg 3
        ]
        self.write_reg(0x00, reg_values[0])
        self.write_reg(0x01, reg_values[1])
        self.write_reg(0x02, reg_values[2])
        self.write_reg(0x03, reg_values[3])
